...
def MeanASequence(model_input, num_frames, num_samples):
  ...
  dim = model_input.get_shape().as_list()[2]
  ...
  def max_sample():
    ...
    tmp = tf.reshape(tmp, [num_samples, intervs[i], dim])
    return tf.reduce_mean(tmp, 1), tf.constant(num_samples)

  def copy_sample():
      return model_inputs[i][:num_samples, :], num_framess[i]

  for i in range(model_input.get_shape().as_list()[0]):
    tmp1, tmp2 = tf.cond(tf.not_equal(intervs[i], 0), max_sample, copy_sample)
    output.append(tmp1)
    length.append(tmp2)
  return tf.stack(output, 0), tf.stack(length, 0)
  ...
  def max_sample():
    start_frame_index = tf.cast(
      tf.multiply(
          ...
          tf.cast(num_framess[i] - intervs[i]*num_samples, tf.float32)), tf.int32)
    tmp = model_inputs[i][start_frame_index[0]:start_frame_index[0]+intervs[i]*num_samples, :]
    tmp = tf.reshape(tmp, [num_samples, intervs[i], 1024+128])
    return tf.reduce_max(tmp, 1), tf.constant(num_samples)
